 ...
 * paths and URLs to match, so the copy works wherever it lands.
 *
 * @author    CraftPulse
 * @package   Typesense
 * @since     5.9.0
...
class ExampleTemplatesController extends Controller
{
    ...
    private function _copy(string $source, string $destination, string $folderName): void
    {
        ...
        // The bundle's paths and URLs are root-relative to its canonical folder
        // name; a rename rewrites every quoted 'typesense/... reference.
        if ($folderName !== 'typesense') {
            foreach (FileHelper::findFiles($temp, ['only' => ['*.twig']]) as $file) {
                $contents = (string)file_get_contents($file);
                FileHelper::writeToFile($file, str_replace("'typesense/", "'{$folderName}/", $contents));
            }
        }
