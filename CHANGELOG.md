...
- **Analytics rules and destination collections now honor the collection prefix setting.** `Analytics::ensureDestination()`, `upsertRule()` (its embedded `source.collections` and `destination.collection` references), and the popular/no-hits readers all resolve their collection names through `Client::prefixedCollectionName()`, the same seam Search and Sync already use, instead of writing and reading raw, unprefixed names. On an install using a collection prefix, this previously wrote analytics rules against a collection that did not exist (or, on a shared Typesense server, against another environment's collection). Since this behavior never shipped in a release, there is nothing to migrate; re-running "Set up rules" (or `configurePersonalizationLog()`) on an existing unreleased build corrects any rule in place under its original name.
